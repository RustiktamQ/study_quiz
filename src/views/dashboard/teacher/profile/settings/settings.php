...
<?php
   ...
   $stmt = mysqli_prepare($connection, "SELECT created_at FROM users WHERE id = ?");
   ...
   if ($row = mysqli_fetch_assoc($result)) {
       $userregdate = date('j M. Y', strtotime($row['created_at']));
   }
         ...
         <div class="flex flex-col bg-white border shadow-sm rounded-xl">
            <div class="p-4 md:p-5">
               ...
               <div class="max-w-sm mt-4 space-y-3">
                  <input type="text" class="transition py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-green-500 focus:ring-green-500 disabled:opacity-50 disabled:pointer-events-none" placeholder="eg. GreatTeacher">
               </div>
               <button type="button" class="whitespace-nowrap mt-4 flex items-center font-semibold border transition-all ease-in duration-75 text-center select-none disabled:shadow-none disabled:opacity-50 disabled:cursor-not-allowed gap-x-1 active:shadow-none text-sm leading-5 rounded-xl py-1.5 h-8 px-8 bg-green-500 text-white border-green-500 hover:border-green-600 disabled:bg-green-400 disabled:border-green-400 hover:bg-green-600 shadow-10 hover:shadow-15" href="/signup">Save</button>
            </div>
         </div>
